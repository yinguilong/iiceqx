...
namespace iiceqx.Provider
{
    ...
    //where T :class
    {
        ...
        public static WriteConcernResult InsertOne<T>(string connectionString, string databaseName, string collectionName, T entity)
        {
            ...
            MongoServer server = new MongoClient(connectionString).GetServer();//MongoServer.GetServers(connectionString);
        ...
        public static IEnumerable<WriteConcernResult> InsertAll<T>(string connectionString, string databaseName, string collectionName, IEnumerable<T> entitys)
        {
            ...
            MongoServer server = new MongoClient(connectionString).GetServer();//new MongoClient(connectionString).GetServer();
        ...
        {
            MongoServer server = new MongoClient(connectionString_Default).GetServer();
            ...
            using (server.RequestStart(database))//开始连接数据库。
            {
                ...
                foreach (T entity in myCursor)//.SetSkip(100).SetLimit(10)是指读取第一百条后的10条数据。
                {
        ...
        public static IEnumerable<T> GetIEnumerable<T>(string collectionName, IMongoQuery query)
        {
            MongoServer server = new MongoClient(connectionString_Default).GetServer();
        ...
        public static List<T> GetGroupItems<T>(string collectionName, IMongoQuery query, string[] groupKeys)
        {
            MongoServer server = new MongoClient(connectionString_Default).GetServer();
            ...
            using (server.RequestStart(database))//开始连接数据库。
            {
                ...
                MongoCollection<BsonDocument> myCollection = database.GetCollection<BsonDocument>(collectionName);
                myCollection.Group(query, new GroupByBuilder(groupKeys), null, null, null);
